...
export default function WorkoutSessionScreen() {
  ...
  const AVG_POWER_W = 215; // placeholder value for now
  ...
  // Remember first start time
  useEffect(() => {
    if (isRunning && startMsRef.current == null) {
  ...
  // Simple elapsed timer (pause/resume)
  useEffect(() => {
    ...
    (async () => {
      ...
      const sub = DeviceMotion.addListener(
        (evt: DeviceMotionMeasurement) => {
          ...
          // 1) Low-pass accel
          axLpRef.current =
            LPF_ALPHA * axLpRef.current + (1 - LPF_ALPHA) * ax;
          ayLpRef.current =
            LPF_ALPHA * ayLpRef.current + (1 - LPF_ALPHA) * ay;
          ax = axLpRef.current;
          ay = ayLpRef.current;
          ...
          // Update acceleration chart (use magnitude)
          setAccelSeries((prev) => {
  ...
  const handleEndAndSave = async () => {
    if (!user) {
      Alert.alert(
        'Sign in required',
        'Please sign in on the Settings tab before saving workouts.'
      );
      return;
    }
    ...
    if (!startMsRef.current) {
      Alert.alert(
        'No start time',
        'We do not have a start time for this workout yet.'
      );
      return;
    }
    ...
    try {
      ...
      const { error } = await supabase.from('workouts').insert({
        user_id: user.id,
        started_at: new Date(startMsRef.current).toISOString(),
        ended_at: new Date().toISOString(),
        duration_seconds: durationSeconds,
        distance_m: distanceM,
        avg_power_w: AVG_POWER_W,
        // id and created_at are filled by defaults in the DB
      });
      ...
      if (error) {
        ...
        Alert.alert('Workout saved', 'Your session has been saved.');
      }
    } finally {
      setSaving(false);
    }
      ...
      <ScrollView contentContainerStyle={styles.screen}>
        ...
        <View style={styles.topBar}>
          ...
          <Pressable
            style={[
              styles.chartBtn,
              !isRunning && { backgroundColor: '#F3F3F3' },
            ]}
        ...
        <View style={styles.metricsRow}>
          {showSplit && (
            <ThemedView style={styles.metricCardSm}>
              <ThemedText style={styles.metricLabelSm}>
                Split (per 500m)
              </ThemedText>
              <ThemedText style={styles.metricValueSm}>
                {splitText}
        ...
        <View style={styles.saveRow}>
          <Pressable
            style={[
              styles.saveButton,
              saving && { opacity: 0.7 },
              !user && { backgroundColor: '#D0D4DE' },
            ]}
            onPress={handleEndAndSave}
            disabled={saving || !user}
          >
            <ThemedText style={styles.saveButtonText}>
              {saving ? 'Saving...' : 'End & Save Workout'}
            </ThemedText>
          </Pressable>
          {!user && (
            <ThemedText style={styles.saveHint}>
              Sign in on the Settings tab to save workouts.
            </ThemedText>
          )}
        </View>
        ...
        {showPowerGraph && (
          <CurveChart title="Power Over Time" data={powerCurve} />
        )}
